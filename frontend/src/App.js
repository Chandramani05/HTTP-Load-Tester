...
function App() {
  ...
  const calculateStatistics = () => {
    ...
    return {
      ...
      percentile90: percentile90.toFixed(3),
      totalRequests,
      totalErrors
    };
  };
  ...
  const performanceDifference = () => {
    const validLatencies = latencies.filter(latency => latency !== null);
    if (validLatencies.length < 2) return 'Not available';
  ...
  const data = Array(maxDataPoints).fill(null).map((_, index) => ({
    ...
    Latency: latencies[index] !== undefined ? latencies[index].toFixed(3) : null,
  ...
  return (
    <div className="container">
      ...
      <form onSubmit={handleSubmit} className="form-grid">
        ...
        <button type="button" onClick={handleFinish} className="finish-button">Finish Test</button>
      </form>
